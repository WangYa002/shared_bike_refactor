#!/usr/bin/env bash
# Build the bike-stack docker image locally (bike-gateway + bike-dispatch
# 双二进制), ship it to the Tencent Cloud host, and restart docker-compose there.
set -euo pipefail

REMOTE="${REMOTE:-ubuntu@124.220.92.243}"
REMOTE_DIR="${REMOTE_DIR:-~/bike}"
GIT_SHA="$(git rev-parse --short HEAD)"
IMG="bike-stack:${GIT_SHA}"

echo "==> Building image ${IMG}"
docker build -t "${IMG}" -f docker/Dockerfile.server .

echo "==> Saving image ($(docker image inspect "${IMG}" --format '{{.Size}}') bytes)"
docker save "${IMG}" | gzip > /tmp/bike-stack.tgz

echo "==> Copying to ${REMOTE}"
ssh "${REMOTE}" "mkdir -p ${REMOTE_DIR}"
scp /tmp/bike-stack.tgz "${REMOTE}:${REMOTE_DIR}/"
...
ssh "${REMOTE}" "cd ${REMOTE_DIR} && \
    docker load < bike-stack.tgz && \
    docker tag ${IMG} bike-stack:latest && \
    docker compose down && \
