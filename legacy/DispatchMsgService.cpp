...
std::queue<iEevent*> DispatchMsgService::response_events;
...
void DispatchMsgService::svc(void* argv) { //由线程调度
	...
	iEevent* ev = (iEevent*)argv;
	if (!dms->svr_exit_) {
		LOG_DEBUG("DispatchMsgService::svc ...\n");
		//printf("DispatchMsgService::svc ...\n");
		iEevent* rsp = dms->process(ev);

		if (rsp) {//有处理好的响应事件
			rsp->dump(std::cout); //可以调整输出的位置
...
i32 DispatchMsgService::enqueue(iEevent* ev)
{
	if (NULL == ev) {
		return -1;
	}
...
iEevent* DispatchMsgService::process(const iEevent* ev)
{
...
iEevent* DispatchMsgService::parseEvent(const char* message, u32 len , u32 eid) {
	if (!message) {
	...
	while (!done) {
		...
		if (!done) {
			if (ev->get_eid() == EVENTID_GET_MOBILE_CODE_RSP) {
				ConnectSession* cs = (ConnectSession*)ev->get_args(); //手机验证码响应
				AssembleHB(cs, ev, EVENTID_GET_MOBILE_CODE_RSP);
			...
			else if (ev->get_eid() == EVENTID_LOGIN_RSP) { //登录响应
				ConnectSession* cs = (ConnectSession*)ev->get_args();
				AssembleHB(cs, ev, EVENTID_LOGIN_RSP);
...
void DispatchMsgService::AssembleHB(ConnectSession* cs, iEevent* ev, i32 eid)
{
