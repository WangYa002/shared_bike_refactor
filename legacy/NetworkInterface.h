...
typedef struct _ConnectSession {
	...
	iEevent* request; //客户端发送的请求事件
	MESSAGE_STATUS req_stat;

	iEevent* response; //服务端发送的响应事件
...
public:
	...
	static void listener_cb(struct evconnlister* listener, evutil_socket_t fd, struct sockaddr* sock, int socklen, void* arg);
